...
function validateCred(cardNo) {
    let sum = 0;
    // Every other digit is doubled and added to sum
    for (let i = cardNo.length - 2; i >= 0; i -= 2) {
        let doubledValue = cardNo[i] * 2;
        if (doubledValue > 9) {
            doubledValue -= 9;
        }
        sum += doubledValue;
    }
    // Add remaining values to sum
    for (let i = cardNo.length - 1; i >= 0; i -= 2) {            
            sum += cardNo[i];
        }
    return sum % 10 === 0;
}
